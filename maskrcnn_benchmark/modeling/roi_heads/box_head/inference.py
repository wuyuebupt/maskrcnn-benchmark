...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, path=None):
        ...
        gt_labels = boxes[0].extra_fields['labels']
        print (gt_labels)
        occlusion_iou = boxes[0].extra_fields['occlusion_iou']
        print (occlusion_iou)
        occlusion_iou = occlusion_iou.cpu().detach().numpy()
        print (occlusion_iou)
        # exit()
        ...
        if self.mode == 0:
            save_name = 'weights/' + path + '_bbox_weight_occlusion.mat'
        elif self.mode == 1:
            save_name = 'weights/' + path + '_bbox_weight_occlusion_.mat'
        else:
            save_name = 'weights/' + path + '_bbox_weight_shoudNotHappen.mat'
        print (save_name)

        sio.savemat(save_name, {'occlusion': occlusion_iou})
